...
public class BaseNetworkTest extends TestCase {
    ...
    protected Object uContractLock = new Object();
...
//        LogPrinter.showDebug(true);
        ...
        for (int k = 0; k < 1; k++) {
            for (int i = 0; i < N; i++) {
                ...
                Parcel parcel = createParcelWithClassU(stepaCoins, stepaPrivateKeys);
...
//        LogPrinter.showDebug(true);
    ...
    public void registerPayingParcelGood() throws Exception {
        ...
        paymentDecreased.getStateData().set("transaction_units", stepaU.getStateData().getIntOrThrow("transaction_units") - 1);
        ...
        assertEquals(stepaU.getStateData().getIntOrThrow("transaction_units") - 1, paymentDecreased.getStateData().getIntOrThrow("transaction_units"));
        assertEquals(stepaU.getStateData().getIntOrThrow("transaction_units") - 4, secondPayment.getStateData().getIntOrThrow("transaction_units"));
...
//        LogPrinter.showDebug(true);
    ...
    public void registerPayingParcelGoodWithContractService() throws Exception {
        ...
        Parcel parcel = ContractsService.createPayingParcel(stepaCoins.getTransactionPack(), stepaU, 3, 7, stepaPrivateKeys, false);
        ...
        assertEquals(stepaU.getStateData().getIntOrThrow("transaction_units") - 10, stepaCoins.getNew().get(0).getStateData().getIntOrThrow("transaction_units"));
    ...
    public void registerPayingParcelGoodWithContractServiceWithTestPayment() throws Exception {
        ...
        Contract stepaU = InnerContractsService.createFreshU(100000000, stepaPublicKeys, true);
        stepaU.setExpiresAt(stepaU.getCreatedAt().plusMonths(6));
        stepaU.seal();
        stepaU.check();
        stepaU.traceErrors();
        node.registerItem(stepaU);
        ...
        Parcel parcel = ContractsService.createPayingParcel(stepaCoins.getTransactionPack(), stepaU, 20, 60, stepaPrivateKeys, true);
        ...
        assertEquals(stepaU.getStateData().getIntOrThrow("test_transaction_units") - 80, stepaCoins.getNew().get(0).getStateData().getIntOrThrow("test_transaction_units"));
...
//    @Test(timeout = 90000)
    public void declineParcelWithTooBigCostUTestPayment() throws Exception {
        ...
        Parcel parcel = ContractsService.createParcel(stepaCoins, stepaU, processedCost, stepaPrivateKeys, true);
...
//        LogPrinter.showDebug(true);
    ...
    public void declineParcelWithTestPaymentBut4096Key() throws Exception {
        ...
        Parcel parcel = ContractsService.createParcel(coins, stepaU, 1, stepaPrivateKeys, true);
...
//        LogPrinter.showDebug(true);
    ...
    public void declineParcelWithTestAndRealPayment() throws Exception {
        ...
        Contract stepaU = InnerContractsService.createFreshU(100, stepaPublicKeys, true);
        stepaU.check();
        //stepaU.setIsU(true);
        stepaU.traceErrors();
        node.registerItem(stepaU);
        ...
        paymentDecreased.getStateData().set("test_transaction_units", stepaU.getStateData().getIntOrThrow("test_transaction_units") - 1);
        paymentDecreased.getStateData().set("transaction_units", stepaU.getStateData().getIntOrThrow("transaction_units") - 1);
        paymentDecreased.seal();
...
//        LogPrinter.showDebug(true);
    ...
    @Test(timeout = 90000)
    public void registerParcelWithTestUButRealPayment() throws Exception {
...
//        registerAndCheckApproved(stepaU);
    ...
    public void declineParcelWithNotSignedPayment() throws Exception {
        ...
        stepaU.traceErrors();
        node.registerItem(stepaU);
        ItemResult itemResult = node.waitItem(stepaU.getId(), 8000);
        assertEquals(ItemState.APPROVED, itemResult.state);
//        registerAndCheckApproved(stepaU);

        Contract paymentDecreased = stepaU.createRevision();
        paymentDecreased.getStateData().set("transaction_units", stepaU.getStateData().getIntOrThrow("transaction_units") - 50);
...
//        }
        ...
        for (Node networkNode: nodes) {
            ...
            Set<Long> envs = networkNode.getLedger().getSubscriptionEnviromentIds(simpleContract.getId());
            if(envs.size() > 0) {
                for(Long envId : envs) {
                    ...
                    for (ContractStorage foundStorage : environment.storages()) {
                        System.out.println(foundStorage.expiresAt());
                        assertAlmostSame(calculateExpires, foundStorage.expiresAt(), 5);
                    }
                }
            } else {
                fail("ContractSubscription was not found");
            }
        ...
        Set<Long> envs = node.getLedger().getSubscriptionEnviromentIds(simpleContract.getId());
        if(envs.size() > 0) {
            for(Long envId : envs) {
        ...
        for (Node networkNode: nodes) {
            envs = networkNode.getLedger().getSubscriptionEnviromentIds(simpleContract.getId());
            if (envs.size() > 0) {
                for (Long envId : envs) {
                    NImmutableEnvironment environment = networkNode.getLedger().getEnvironment(envId);
                    for (ContractSubscription foundCss : environment.subscriptions()) {
                    ...
                    for (ContractStorage foundStorage : environment.storages()) {
                        System.out.println(foundStorage.expiresAt());
                        assertAlmostSame(calculateExpires, foundStorage.expiresAt(),
                                (long) (5 * config.getServiceRate(NSmartContract.SmartContractType.SLOT1.name()).doubleValue()));
                    }
                }
            } else {
        ...
        envs = node.getLedger().getSubscriptionEnviromentIds(simpleContract.getId());
        if(envs.size() > 0) {
            for(Long envId : envs) {
                NImmutableEnvironment environment = node.getLedger().getEnvironment(envId);
                for (ContractSubscription foundCss : environment.subscriptions()) {
    ...
    public void registerSlotContractWithStoringRevisions() throws Exception {
        ...
        for (Node networkNode: nodes) {
            ...
            if (envs.size() > 0) {
                for (Long envId : envs) {
                    ...
                    for (ContractStorage foundStorage : environment.storages()) {
                        System.out.println("expected " + calculateExpires);
                        System.out.println("found " + foundStorage.expiresAt());
                        assertAlmostSame(calculateExpires, foundStorage.expiresAt(), 5);
                    }
                }
            } else {
        ...
        for (Node networkNode: nodes) {
            ...
            envs = networkNode.getLedger().getSubscriptionEnviromentIds(simpleContract2.getId());
            if(envs.size() > 0) {
                for(Long envId : envs) {
                    NImmutableEnvironment environment = networkNode.getLedger().getEnvironment(envId);
                    for (ContractSubscription foundCss : environment.subscriptions()) {
        ...
        envs = node.getLedger().getSubscriptionEnviromentIds(simpleContract2.getId());
        if(envs.size() > 0) {
            for(Long envId : envs) {
                NImmutableEnvironment environment = node.getLedger().getEnvironment(envId);
                for (ContractSubscription foundCss : environment.subscriptions()) {
        ...
        for (Node networkNode: nodes) {
            ...
            restoredPackedData = networkNode.getLedger().getContractInStorage(simpleContract3.getId());
            assertNotNull(restoredPackedData);
            restoredContract = Contract.fromPackedTransaction(restoredPackedData);
            assertNotNull(restoredContract);
            assertEquals(simpleContract3.getId(), restoredContract.getId());

            envs = networkNode.getLedger().getSubscriptionEnviromentIds(simpleContract3.getId());
            if(envs.size() > 0) {
                for(Long envId : envs) {
                    NImmutableEnvironment environment = networkNode.getLedger().getEnvironment(envId);
                    for (ContractSubscription foundCss : environment.subscriptions()) {
...
//        calculateExpires = timeReg2.plusSeconds(((100 + 300 + 300) * config.getServiceRate(NSmartContract.SmartContractType.SLOT1.name()).doubleValue() * 1024 * 24 * 3600 - (long)spentKDs) /
//                (simpleContract3.getPackedTransaction().length + simpleContract2.getPackedTransaction().length));
        ...
        envs = node.getLedger().getSubscriptionEnviromentIds(simpleContract3.getId());
        if(envs.size() > 0) {
            for(Long envId : envs) {
                NImmutableEnvironment environment = node.getLedger().getEnvironment(envId);
                for (ContractSubscription foundCss : environment.subscriptions()) {
        ...
        for (Node networkNode: nodes) {
            ...
            assertNotNull(networkNode.getLedger().getEnvironment(refilledSlotContract3.getId()));
        }
...
//        calculateExpires = timeReg3.plusSeconds(((100 + 300 + 300 + 300) * config.getServiceRate(NSmartContract.SmartContractType.SLOT1.name()).doubleValue() * 1024 * 24 * 3600 - (long) spentKDs) / simpleContract3.getPackedTransaction().length);
        ...
        for (Node networkNode: nodes) {
            ...
            assertNull(networkNode.getLedger().getEnvironment(refilledSlotContract3.getId()));
        }
...
//                (simpleContract3.getPackedTransaction().length + simpleContract2.getPackedTransaction().length));
    ...
    public void registerSlotContractInNewItem() throws Exception {
        ...
        for (Node networkNode: nodes) {
            ...
            assertEquals(0, networkNode.getLedger().getSubscriptionEnviromentIds(simpleContract.getId()).size());
            assertEquals(0, networkNode.getLedger().getSubscriptionEnviromentIds(simpleContract2.getId()).size());
            assertEquals(0, networkNode.getLedger().getSubscriptionEnviromentIds(slotContract.getId()).size());
            assertEquals(0, networkNode.getLedger().getSubscriptionEnviromentIds(refilledSlotContract.getId()).size());
        ...
        assertEquals(0, node.getLedger().getSubscriptionEnviromentIds(simpleContract.getId()).size());
        assertEquals(0, node.getLedger().getSubscriptionEnviromentIds(simpleContract2.getId()).size());
        assertEquals(0, node.getLedger().getSubscriptionEnviromentIds(slotContract.getId()).size());
        assertEquals(0, node.getLedger().getSubscriptionEnviromentIds(refilledSlotContract.getId()).size());
    ...
    @Test
    public void registerTwoSlotContracts() throws Exception {
        final PrivateKey key = new PrivateKey(Do.read(ROOT_PATH + "_xer0yfe2nn1xthc.private.unikey"));
        Set<PrivateKey> slotIssuerPrivateKeys = new HashSet<>();
        slotIssuerPrivateKeys.add(key);
        Set<PublicKey> slotIssuerPublicKeys = new HashSet<>();
        ...
        // second slot contract that storing

        SlotContract slotContract2 = ContractsService.createSlotContract(slotIssuerPrivateKeys, slotIssuerPublicKeys, nodeInfoProvider);
        slotContract2.putTrackingContract(simpleContract);
        ...
        payingParcel = ContractsService.createPayingParcel(slotContract2.getTransactionPack(), paymentContract, 1, 100, stepaPrivateKeys, false);

        slotContract2.check();
        slotContract2.traceErrors();
        assertTrue(slotContract2.isOk());

        assertEquals(NSmartContract.SmartContractType.SLOT1.name(), slotContract2.getDefinition().getExtendedType());
        assertEquals(NSmartContract.SmartContractType.SLOT1.name(), slotContract2.get("definition.extended_type"));
        assertEquals(100 * config.getServiceRate(NSmartContract.SmartContractType.SLOT1.name()).doubleValue(), slotContract2.getPrepaidKilobytesDays().doubleValue(), 0.01);
        ...
        assertEquals(ItemState.REVOKED, node.waitItem(payingParcel.getPayment().getContract().getId(), 8000).state);
        assertEquals(ItemState.APPROVED, node.waitItem(payingParcel.getPayload().getContract().getId(), 8000).state);
        assertEquals(ItemState.APPROVED, node.waitItem(slotContract2.getNew().get(0).getId(), 8000).state);

        itemResult = node.waitItem(slotContract2.getId(), 8000);
        assertEquals("ok", itemResult.extraDataBinder.getBinder("onCreatedResult").getString("status", null));

        assertEquals(simpleContract.getId(), slotContract2.getTrackingContract().getId());
        assertEquals(simpleContract.getId(), ((SlotContract) payingParcel.getPayload().getContract()).getTrackingContract().getId());

        Thread.sleep(5000);

        days = (double) 100 * config.getServiceRate(NSmartContract.SmartContractType.SLOT1.name()).doubleValue() * 1024 / simpleContract.getPackedTransaction().length;
        seconds = (long) (days * 24 * 3600);
        calculateExpires = timeReg2.plusSeconds(seconds);
        ...
        Set<Long> envs2 = node.getLedger().getSubscriptionEnviromentIds(simpleContract.getId());
        if(envs.size() > 0) {
            for(Long envId : envs2)
        ...
        // check if we store environment
        assertNotNull(node.getLedger().getEnvironment(slotContract2.getId()));
    }
    ...
    public void registerUnsContract() throws Exception {
        ...
        assertEquals(ItemState.APPROVED, irPayload.state);
        assertEquals(ItemState.REVOKED, node.waitItem(payingParcel.getPayment().getContract().getId(), 8000).state);
        ...
        // additional check for all network nodes
        nodes.forEach(n -> assertEquals(n.getLedger().getNameEntries(name).size(),2));

        assertEquals(ledger.getNameEntries(name).size(),2);
    }
    ...
    public void checkUnsContractHOLD() throws Exception {
        ...
        Thread.sleep(5000);
        // additional check for all network nodes
        nodes.forEach(n -> assertEquals(n.getLedger().getNameEntries(name).size(),1));

        assertEquals(ledger.getNameEntries(name).size(),1);
        nodes.forEach((n) -> n.getLedger().clearExpiredNameRecords(config.getHoldDuration()));
        Thread.sleep(11000);
        nodes.forEach((n) -> n.getLedger().clearExpiredNameRecords(config.getHoldDuration()));
        NNameRecord nr = ledger.getNameRecord(name);
        List<NNameRecordEntry> nrEntries = ledger.getNameEntries(name);
        assertEquals(nrEntries.size(),1);
    ...
    public void registerUnsContractRevision() throws Exception {
        ...
        String nameToChange = "change"+Instant.now().getEpochSecond();
        String nameToAdd = "add"+Instant.now().getEpochSecond();
        String nameToRemove = "remove"+Instant.now().getEpochSecond();
...
*/
    ...
    public void registerUnsContractFromDsl() throws Exception {
        ...
        UnsName unsName = uns.getName("test_from_dsl");
        uns.removeAddress(uns.getAddresses().iterator().next());
        unsName.setUnsName("test_from_dsl" + Instant.now().getEpochSecond());
        unsName.setUnsReducedName("test_from_dsl" + Instant.now().getEpochSecond());
        uns.addKey(randomPrivKey.getPublicKey());
    ...
    public void checkUnsContractForBusyName() throws Exception{
        ...
        uns1.addName(name,name, "testname description");
        ...
        assertErrorsContainsSubstr(irPayload.errors, "name '"+name+"' is not available");
        assertEquals(ItemState.APPROVED, node.waitItem(payingParcel.getPayment().getContract().getId(), 8000).state);
        assertEquals(ItemState.UNDEFINED, node.waitItem(uns2.getNew().get(0).getId(), 8000).state);
    ...
    public void checkUnsContractForBusyAddress() throws Exception{
        ...
        ItemResult irPayload = node.waitItem(payingParcel.getPayload().getContract().getId(), 8000);
        assertEquals(ItemState.DECLINED, irPayload.state);
        assertErrorsContainsSubstr(irPayload.errors, "address '"+randomPrivateKey.getPublicKey().getShortAddress()+"' is not available");
        assertErrorsContainsSubstr(irPayload.errors, "address '"+randomPrivateKey.getPublicKey().getLongAddress()+"' is not available");
        assertEquals(ItemState.APPROVED, node.waitItem(payingParcel.getPayment().getContract().getId(), 8000).state);
        assertEquals(ItemState.UNDEFINED, node.waitItem(uns2.getNew().get(0).getId(), 8000).state);
    ...
    public void checkUnsContractForBusyOrigin() throws Exception{
        ...
        uns1.addName(name,name,"");
        uns1.addKey(randomPrivateKey1.getPublicKey());
        ...
        ItemResult irPayload = node.waitItem(payingParcel.getPayload().getContract().getId(), 8000);
        assertEquals(ItemState.DECLINED, irPayload.state);
        assertErrorsContainsSubstr(irPayload.errors, "origin '"+nameContract.getOrigin().toBase64String()+"' is not available");
        assertEquals(ItemState.APPROVED, node.waitItem(payingParcel.getPayment().getContract().getId(), 8000).state);
        assertEquals(ItemState.UNDEFINED, node.waitItem(uns2.getNew().get(0).getId(), 8000).state);
    }
    ...
    public void checkUnsContractForExpiresTime() throws Exception {
        ...
        uns.addName(reducedName,reducedName,"");
        uns.addOrigin(referencesContract);
        uns.addKey(randomPrivKey.getPublicKey());
        ...
        assertEquals(ledger.getNameEntries(reducedName).size(), 2);

        // check calculation expiration time
        double days = (double) 1470 * config.getServiceRate(NSmartContract.SmartContractType.UNS1.name()).doubleValue() / uns.getNames().size();
        long seconds = (long) (days * 24 * 3600);
        ZonedDateTime calculateExpires = timeReg1.plusSeconds(seconds);
...
//        assertEquals("ok", itemResult.extraDataBinder.getBinder("onUpdateResult").getString("status", null));
    ...
    public void checkPaymentStatisticsWithApprovedParcelAndRealPayment() throws Exception {
        ...
        Set<PublicKey> keys = new HashSet();
        keys.add(ownerKey.getPublicKey());
        Contract contractU = InnerContractsService.createFreshU(100, keys, true);
        contractU.check();
        //stepaU.setIsU(true);
        contractU.traceErrors();
        node.registerItem(contractU);
        ItemResult itemResult = node.waitItem(contractU.getId(), 18000);
        assertEquals(ItemState.APPROVED, itemResult.state);

        Parcel parcel = ContractsService.createParcel(contract, contractU, 1, stepaPrivateKeys, false);
...
//        assertEquals(node.nodeStats.todayPaidAmount - today, 1);
    ...
    public void checkPaymentStatisticsWithApprovedParcelAndTestPayment() throws Exception {
        ...
        Parcel parcel = ContractsService.createParcel(contract, contractU, 1, stepaPrivateKeys, true);
...
//        assertEquals(node.nodeStats.todayPaidAmount - today, 0);
    ...
    public void checkPaymentStatisticsWithDeclinedParcel() throws Exception {
        ...
        PrivateKey manufacturePrivateKey = new PrivateKey(Do.read(ROOT_PATH + "_xer0yfe2nn1xthc.private.unikey"));
        Contract contractU = Contract.fromDslFile(ROOT_PATH + "StepaU.yml");
        contractU.addSignerKey(manufacturePrivateKey);
        contractU.seal();
        contractU.check();
        contractU.traceErrors();

        Parcel parcel = ContractsService.createParcel(contract, contractU, 1, stepaPrivateKeys);
...
//        assertEquals(node.nodeStats.todayPaidAmount - today, 1801);
    ...
    public void checkPaymentStatisticsWithDeclinedPayingParcelUns() throws Exception {
        ...
        // illegal payment contract
        Contract contractU = Contract.fromDslFile(ROOT_PATH + "StepaU.yml");
        contractU.addSignerKey(manufacturePrivateKeys.iterator().next());
        contractU.seal();
        contractU.check();
        contractU.traceErrors();

        parcel = ContractsService.createPayingParcel(uns.getTransactionPack(), contractU, 1, 1500, stepaPrivateKeys, false);
    ...
    public void removeEnvironment_uns1() throws Exception {
        ...
        Parcel parcel = ContractsService.createParcel(referencesContract.getTransactionPack(), paymentContract, 1, stepaPrivateKeys, false);
        node.registerParcel(parcel);
        synchronized (uContractLock) {uContract = parcel.getPaymentContract();}
        node.waitParcel(parcel.getId(), 8000);
        assertEquals(ItemState.APPROVED, node.waitItem(referencesContract.getId(), 8000).state);
    ...
    public void removeEnvironment_slot1() throws Exception {
        ...
        for (Node n : nodes) {
            ...
            PreparedStatement ps = ((PostgresLedger)n.getLedger()).getDb().statement(
                    "select count(*) from contract_subscription where hash_id = ?",
                    env.subscriptions().iterator().next().getContractId().getDigest());
            ResultSet rs = ps.executeQuery();
            assertTrue(rs.next());
    ...
    public void ecsrowComplete() throws Exception {
        ...
        issuerPrivateKeys.add(TestKeys.privateKey(0));
        arbitratorPrivateKeys.add(new PrivateKey(Do.read(ROOT_PATH + "_xer0yfe2nn1xthc.private.unikey")));
        customerPrivateKeys.add(new PrivateKey(Do.read(ROOT_PATH + "keys/stepan_mamontov.private.unikey")));
        executorPrivateKeys.add(new PrivateKey(Do.read(ROOT_PATH + "keys/marty_mcfly.private.unikey")));
        ...
        payment.check();
        payment.traceErrors();
        registerAndCheckApproved(payment);
        ...
        registerAndCheckApproved(escrow);

        ItemResult itemResult = node.waitItem(payment.getId(), 8000);
        assertEquals(ItemState.APPROVED, itemResult.state);
        ...
        registerAndCheckApproved(completedEscrow);
        ...
        registerAndCheckApproved(newPayment);
    }
    ...
    public void ecsrowCancel() throws Exception {
        ...
        // register signed escrow contract
        registerAndCheckApproved(canceledEscrow);
    ...
    public void ecsrowCompleteFail() throws Exception {
        ...
        registerAndCheckDeclined(completedEscrow);
    ...
    public void ecsrowCancelFail() throws Exception {
        ...
        registerAndCheckDeclined(canceledEscrow);
    ...
    public void ecsrowReOpenFail() throws Exception {
        ...
        registerAndCheckDeclined(revisionEscrow);
    ...
    public void registerFollowerContract() throws Exception {
        ...
        Set<Long> envs = node.getLedger().getSubscriptionEnviromentIds(simpleContract.getOrigin());
        if(envs.size() > 0) {
            for(Long envId : envs) {
                NImmutableEnvironment environment = node.getLedger().getEnvironment(envId);
                for (ContractSubscription foundCss : environment.subscriptions()) {
        ...
        // additional check for all network nodes
        for (Node networkNode: nodes) {
            envs = networkNode.getLedger().getSubscriptionEnviromentIds(simpleContract.getOrigin());
            if(envs.size() > 0) {
                for(Long envId : envs) {
                    NImmutableEnvironment environment = networkNode.getLedger().getEnvironment(envId);
                    for (ContractSubscription foundCss : environment.subscriptions()) {
        ...
        // refill follower contract with U (means add following days or number of sended callbacks).

        FollowerContract refilledFollowerContract = (FollowerContract) followerContract.createRevision(key);
        refilledFollowerContract.setNodeInfoProvider(nodeInfoProvider);

        // payment contract
        // will create two revisions in the createPayingParcel, first is pay for register, second is pay for following
        paymentContract = getApprovedUContract();

        payingParcel = ContractsService.createPayingParcel(refilledFollowerContract.getTransactionPack(), paymentContract, 1, 300, stepaPrivateKeys, false);

        refilledFollowerContract.check();
        refilledFollowerContract.traceErrors();
        assertTrue(refilledFollowerContract.isOk());
        ...
        assertEquals(ItemState.REVOKED, node.waitItem(payingParcel.getPayment().getContract().getId(), 8000).state);
        assertEquals(ItemState.APPROVED, node.waitItem(payingParcel.getPayload().getContract().getId(), 8000).state);
        assertEquals(ItemState.APPROVED, node.waitItem(refilledFollowerContract.getNew().get(0).getId(), 8000).state);

        itemResult = node.waitItem(refilledFollowerContract.getId(), 8000);
        assertEquals("ok", itemResult.extraDataBinder.getBinder("onUpdateResult").getString("status", null));
        ...
        envs = node.getLedger().getSubscriptionEnviromentIds(simpleContract.getOrigin());
        if(envs.size() > 0) {
            for(Long envId : envs) {
                NImmutableEnvironment environment = node.getLedger().getEnvironment(envId);
                for (ContractSubscription foundCss : environment.subscriptions()) {
        ...
        // refill follower contract with U again (means add following days or number of sended callbacks). The oldest revision should removed

        FollowerContract refilledFollowerContract2 = (FollowerContract) refilledFollowerContract.createRevision(key);
        refilledFollowerContract2.setNodeInfoProvider(nodeInfoProvider);

        // payment contract
        // will create two revisions in the createPayingParcel, first is pay for register, second is pay for following
        paymentContract = getApprovedUContract();

        payingParcel = ContractsService.createPayingParcel(refilledFollowerContract2.getTransactionPack(), paymentContract, 1, 300, stepaPrivateKeys, false);

        refilledFollowerContract2.check();
        refilledFollowerContract2.traceErrors();
        assertTrue(refilledFollowerContract2.isOk());
        ...
        assertEquals(ItemState.REVOKED, node.waitItem(payingParcel.getPayment().getContract().getId(), 8000).state);
        assertEquals(ItemState.APPROVED, node.waitItem(payingParcel.getPayload().getContract().getId(), 8000).state);
        assertEquals(ItemState.APPROVED, node.waitItem(refilledFollowerContract2.getNew().get(0).getId(), 8000).state);

        itemResult = node.waitItem(refilledFollowerContract2.getId(), 8000);
        assertEquals("ok", itemResult.extraDataBinder.getBinder("onUpdateResult").getString("status", null));
        ...
        assertEquals(ItemState.REVOKED, node.waitItem(refilledFollowerContract.getId(), 8000).state);

        // revoke follower contract, means remove stored contract from storage

        Contract revokingFollowerContract = ContractsService.createRevocation(refilledFollowerContract2, key);

        registerAndCheckApproved(revokingFollowerContract);
    ...
    private Contract checkFollowerContractRevision(Contract contractRevision, PrivateKey oldOwnerKey, PrivateKey newOwnerKey,
                                                   int callbackNum, ZonedDateTime baseCalculateExpires, HashId originCallback,
                                                   HashId originNoCallback, int origins, boolean waitCallback,
                                                   boolean waitRevokeCallback, boolean checkSubscription) throws Exception {
        // create revision of follow contract
        Contract simpleContractRevision = contractRevision.createRevision(oldOwnerKey);
        simpleContractRevision.setOwnerKeys(newOwnerKey);
        ...
        // find call follower callbacks
        assertTrue((output.indexOf("Follower callback received. Contract: " + simpleContractRevision.getId().toString()) >= 0) == waitCallback);
        assertTrue((output.indexOf("Follower callback processed. Contract: " + simpleContractRevision.getId().toString()) >= 0) == waitCallback);
        assertTrue((output.indexOf("Follower callback received. Revoking ID: " + contractRevision.getId().toString()) >= 0) == waitRevokeCallback);
        assertTrue((output.indexOf("Follower callback processed. Revoking ID: " + contractRevision.getId().toString()) >= 0) == waitRevokeCallback);
        ...
        if (originNoCallback != null) {
            envs = node.getLedger().getSubscriptionEnviromentIds(originNoCallback);
            if (envs.size() > 0) {
                for (Long envId : envs) {
                    NImmutableEnvironment environment = node.getLedger().getEnvironment(envId);
                    for (ContractSubscription foundCss : environment.subscriptions()) {
        ...
        envs = node.getLedger().getSubscriptionEnviromentIds(originCallback);
        if(envs.size() > 0) {
            for(Long envId : envs) {
                NImmutableEnvironment environment = node.getLedger().getEnvironment(envId);
                for (ContractSubscription foundCss : environment.subscriptions()) {
                ...
                System.out.println("expected callbacks spent: " + callbackRate * callbackNum);
                System.out.println("found callbacks spent: " + fs.getCallbacksSpent());
                assertEquals(callbackRate * callbackNum, fs.getCallbacksSpent(), 0.001);
            }
        } else {
            fail("FollowerSubscription was not found");
        }

        if (waitCallback || waitRevokeCallback)
            Thread.sleep(5000);

        // additional check for all network nodes
        for (Node networkNode: nodes) {
            if (originNoCallback != null) {
                envs = networkNode.getLedger().getSubscriptionEnviromentIds(originNoCallback);
                if (envs.size() > 0) {
                    for (Long envId : envs) {
                        NImmutableEnvironment environment = networkNode.getLedger().getEnvironment(envId);
                        for (ContractSubscription foundCss : environment.subscriptions()) {
            ...
            envs = networkNode.getLedger().getSubscriptionEnviromentIds(originCallback);
            if(envs.size() > 0) {
                for(Long envId : envs) {
                    NImmutableEnvironment environment = networkNode.getLedger().getEnvironment(envId);
                    for (ContractSubscription foundCss : environment.subscriptions()) {
    ...
    @Test
    public void registerFollowerContractRevisions() throws Exception {
        ...
        envs = node.getLedger().getSubscriptionEnviromentIds(simpleContract2.getOrigin());
        if(envs.size() > 0) {
            for(Long envId : envs) {
                NImmutableEnvironment environment = node.getLedger().getEnvironment(envId);
                for (ContractSubscription foundCss : environment.subscriptions()) {
        ...
        for (Node networkNode: nodes) {
            ...
            envs = networkNode.getLedger().getSubscriptionEnviromentIds(simpleContract2.getOrigin());
            if(envs.size() > 0) {
                for(Long envId : envs) {
                    NImmutableEnvironment environment = networkNode.getLedger().getEnvironment(envId);
                    for (ContractSubscription foundCss : environment.subscriptions()) {
        ...
        // register 6 revisions of following contract
        Contract simpleContractRevision = simpleContract2;
        for (int i = 0; i < 3; i++) {
            simpleContractRevision = checkFollowerContractRevision(simpleContractRevision, key2, key, i * 2 + 1,
                    calculateExpires, simpleContract2.getOrigin(), simpleContract.getOrigin(),
                    newRevFollowerContract.getTrackingOrigins().size(), true, false, true);
            simpleContractRevision = checkFollowerContractRevision(simpleContractRevision, key, key2, (i + 1) * 2,
                    calculateExpires, simpleContract2.getOrigin(), simpleContract.getOrigin(),
                    newRevFollowerContract.getTrackingOrigins().size(), false, true, true);
        }

        // subscriptions already muted (after 7 callback if callback cost 50 U)
        ...
        simpleContractRevision = checkFollowerContractRevision(simpleContractRevision, key, key2, 7,
                calculateExpires, simpleContract2.getOrigin(), simpleContract.getOrigin(),
                newRevFollowerContract.getTrackingOrigins().size(), false, false, false);

        // remaining payments are not enough to pay callback
        simpleContractRevision = checkFollowerContractRevision(simpleContractRevision, key2, key, 8,
                calculateExpires, simpleContract2.getOrigin(), simpleContract.getOrigin(),
                newRevFollowerContract.getTrackingOrigins().size(), false, false, false);
    ...
    @Test
    public void checkFollowerCallbackWrongSignature() throws Exception {
        ...
        // callback key
        PrivateKey callbackKey = new PrivateKey(2048);
        PrivateKey wrongCallbackKey = new PrivateKey(2048);
        ...
        String output = ConsoleInterceptor.copyOut(() -> {
            registerAndCheckApproved(simpleContractRevision);
            Thread.sleep(3000 * (nodes.size() + 1));
        });
        System.out.println(output);

        // check call follower callbacks
        assertTrue(output.indexOf("Follower callback received. Contract: " + simpleContractRevision.getId().toString()) >= 0);
        assertTrue(output.indexOf("Follower callback processed. Contract: " + simpleContractRevision.getId().toString()) >= 0);
        assertTrue(output.indexOf("Follower callback received. Revoking ID: " + simpleContract.getId().toString()) >= 0);
        assertTrue(output.indexOf("Follower callback processed. Revoking ID: " + simpleContract.getId().toString()) >= 0);
        ...
        if(envs.size() > 0) {
            for(Long envId : envs) {
                ...
                System.out.println("expected mutedAt: " + calculateExpires.minusSeconds(seconds * 3));      // callbacks already started
                System.out.println("found mutedAt: " + fs.mutedAt());
        ...
        // intercept System.out
        output = ConsoleInterceptor.copyOut(() -> Thread.sleep(3000 * (nodes.size() + 1)));
        System.out.println(output);

        // check don`t call follower callbacks
        assertFalse(output.indexOf("Follower callback received. Contract: " + simpleContractRevision.getId().toString()) >= 0);
        assertFalse(output.indexOf("Follower callback processed. Contract: " + simpleContractRevision.getId().toString()) >= 0);
        assertFalse(output.indexOf("Follower callback received. Revoking ID: " + simpleContract.getId().toString()) >= 0);
        assertFalse(output.indexOf("Follower callback processed. Revoking ID: " + simpleContract.getId().toString()) >= 0);

        // check callback completed
        seconds = (long) (callbackRate * 24 * 3600);
        calculateExpires = calculateExpires.minusSeconds(seconds * 2);
    ...
    @Test
    public void checkFollowerCallbackFailed() throws Exception {
        ...
        // configuration for test
        nodes.forEach(n -> {
            n.getConfig().setFollowerCallbackDelay(Duration.ofSeconds(3));
            n.getConfig().setFollowerCallbackExpiration(Duration.ofSeconds(40));
        });

        // init follower callback (on wrong port)
        FollowerCallback callback = new FollowerCallback(callbackKey, 8888, "/follow.callback");
    ...
    @Test
    public void checkFollowerCallbackNodeKeys() throws Exception {
        ...
        Contract simpleContractRevision = checkFollowerContractRevision(simpleContract, key2, key, 1, calculateExpires,
                simpleContract.getOrigin(), null, followerContract.getTrackingOrigins().size(), true, false, true);
        simpleContractRevision = checkFollowerContractRevision(simpleContractRevision, key, key2, 2, calculateExpires,
                simpleContract.getOrigin(), null, followerContract.getTrackingOrigins().size(), false, true, true);

        // check without network nodes keys
        callback.clearNetworkNodeKeys();

        simpleContractRevision = checkFollowerContractRevision(simpleContractRevision, key2, key, 3, calculateExpires,
                simpleContract.getOrigin(), null, followerContract.getTrackingOrigins().size(), true, false, true);
        simpleContractRevision = checkFollowerContractRevision(simpleContractRevision, key, key2, 4, calculateExpires,
                simpleContract.getOrigin(), null, followerContract.getTrackingOrigins().size(), false, true, true);

        // check with wrong network nodes keys
        Set<PublicKey> wrongKeys =  new HashSet<>();
        wrongKeys.add(new PrivateKey(2048).getPublicKey());
        callback.setNetworkNodeKeys(wrongKeys);

        // create revision of follow contract
        Contract finalContractRevision = simpleContractRevision.createRevision(key2);
        finalContractRevision.setOwnerKeys(key);
        finalContractRevision.seal();
        finalContractRevision.check();
        finalContractRevision.traceErrors();
        assertTrue(finalContractRevision.isOk());

        // intercept System.out
        String output = ConsoleInterceptor.copyOut(() -> {
            registerAndCheckApproved(finalContractRevision);
            Thread.sleep(3000 * (nodes.size() + 1));
        });
        System.out.println(output);

        // check call follower callback
        assertTrue(output.indexOf("Follower callback received. Contract: " + finalContractRevision.getId().toString()) >= 0);
        assertFalse(output.indexOf("Follower callback processed. Contract: " + finalContractRevision.getId().toString()) >= 0);

        // check callback not completed
        calculateExpires = calculateExpires.minusSeconds(seconds * 4);
        ...
        if(envs.size() > 0) {
            for(Long envId : envs) {
                ...
                System.out.println("expected mutedAt: " + calculateExpires.minusSeconds(seconds * 2));      // callback already started
                System.out.println("found mutedAt: " + fs.mutedAt());
                assertAlmostSame(calculateExpires.minusSeconds(seconds * 2), fs.mutedAt(), 5);

                System.out.println("expected started callbacks: 1");
                System.out.println("found started callbacks: " + fs.getStartedCallbacks());
                assertEquals(1, fs.getStartedCallbacks());

                System.out.println("expected callbacks spent: " + callbackRate * 4);
                System.out.println("found callbacks spent: " + fs.getCallbacksSpent());
                assertEquals(callbackRate * 4, fs.getCallbacksSpent(), 0.001);
            }
        } else {
            fail("FollowerSubscription was not found");
        }
        ...
        // check call follower callback
        assertFalse(output.indexOf("Follower callback received. Contract: " + finalContractRevision.getId().toString()) >= 0);
        assertFalse(output.indexOf("Follower callback processed. Contract: " + finalContractRevision.getId().toString()) >= 0);
        ...
        if(envs.size() > 0) {
            for(Long envId : envs) {
                ...
                System.out.println("expected callbacks spent: " + callbackRate * 5);
                System.out.println("found callbacks spent: " + fs.getCallbacksSpent());
                assertEquals(callbackRate * 5, fs.getCallbacksSpent(), 0.001);
            }
        } else {
            fail("FollowerSubscription was not found");
        }
    ...
    @Test
    public void registerFollowerContractAndCallbacks() throws Exception {
        ...
        FollowerContract followerContract = ContractsService.createFollowerContract(followerIssuerPrivateKeys,
                followerIssuerPublicKeys, nodeInfoProvider);
        followerContract.putTrackingOrigin(simpleContract.getOrigin(), "http://localhost:7787/follow.callback",
                callbackKey.getPublicKey());
        ...
        assertEquals(followerContract.getCallbackKeys().get("http://localhost:7787/follow.callback"),callbackKey.getPublicKey());
        assertEquals(followerContract.getTrackingOrigins().get(simpleContract.getOrigin()), "http://localhost:7787/follow.callback");
        assertTrue(followerContract.isOriginTracking(simpleContract.getOrigin()));
        assertTrue(followerContract.isCallbackURLUsed("http://localhost:7787/follow.callback"));
        ...
        // init follower callback
        FollowerCallback callback = new FollowerCallback(callbackKey, 7787, "/follow.callback");

        callback.setNetworkNodeKeys(nodes.stream().map(n -> n.getNodeKey()).collect(Collectors.toSet()));

        // register revision of following contract
        checkFollowerContractRevision(simpleContract, key2, key, 1, calculateExpires, simpleContract.getOrigin(), null,
                followerContract.getTrackingOrigins().size(), true, false, true);

        // contract2 for follow
        Contract simpleContract2 = new Contract(key2);
        simpleContract2.addPermission(changeOwnerPerm);
        simpleContract2.addPermission(revokePermission);
        simpleContract2.seal();
        simpleContract2.check();
        simpleContract2.traceErrors();
        assertTrue(simpleContract2.isOk());
        ...
        newRevFollowerContract.putTrackingOrigin(simpleContract2.getOrigin(), "http://localhost:7787/follow.callback", callbackKey.getPublicKey());
        newRevFollowerContract.setNodeInfoProvider(nodeInfoProvider);
        ...
        assertEquals(newRevFollowerContract.getCallbackKeys().get("http://localhost:7787/follow.callback"), callbackKey.getPublicKey());
        assertEquals(newRevFollowerContract.getTrackingOrigins().get(simpleContract.getOrigin()), "http://localhost:7787/follow.callback");
        assertTrue(newRevFollowerContract.isOriginTracking(simpleContract.getOrigin()));
        assertTrue(newRevFollowerContract.isCallbackURLUsed("http://localhost:7787/follow.callback"));

        assertEquals(newRevFollowerContract.getCallbackKeys().get("http://localhost:7787/follow.callback"),callbackKey.getPublicKey());
        assertEquals(newRevFollowerContract.getTrackingOrigins().get(simpleContract2.getOrigin()), "http://localhost:7787/follow.callback");
        assertTrue(newRevFollowerContract.isOriginTracking(simpleContract2.getOrigin()));
        assertTrue(newRevFollowerContract.isCallbackURLUsed("http://localhost:7787/follow.callback"));
        ...
        if(envs.size() > 0) {
            for(Long envId : envs) {
                ...
                System.out.println("found callbacks spent: " + fs.getCallbacksSpent());
                assertEquals(callbackRate, fs.getCallbacksSpent(), 0.001);
            }
        } else {
            fail("FollowerSubscription was not found");
        }

        Set<Long> envs2 = node.getLedger().getSubscriptionEnviromentIds(simpleContract2.getOrigin());
        assertTrue(envs.equals(envs2));
        ...
        for (Node networkNode: nodes) {
            ...
            envs2 = networkNode.getLedger().getSubscriptionEnviromentIds(simpleContract2.getOrigin());
            assertTrue(envs.equals(envs2));
        ...
        // register revision of following contract
        Contract simpleContractRevision = checkFollowerContractRevision(simpleContract2, key2, key, 1, calculateExpires,
                simpleContract2.getOrigin(), null, followerContract.getTrackingOrigins().size(), false, false, false);

        // new revision of follower contract
        newRevFollowerContract = (FollowerContract) newRevFollowerContract.createRevision(key);
        ...
        if(envs.size() > 0) {
            for(Long envId : envs) {
                ...
                System.out.println("expected mutedAt: " + calculateExpires.minusSeconds(seconds * 2));      // callback started
                System.out.println("found mutedAt: " + fs.mutedAt());
                assertAlmostSame(calculateExpires.minusSeconds(seconds * 2), fs.mutedAt(), 5);
        ...
        envs2 = node.getLedger().getSubscriptionEnviromentIds(simpleContract2.getOrigin());
        assertTrue(envs.equals(envs2));
        ...
        callback = new FollowerCallback(callbackKey, 7787, "/follow.callback");

        // intercept System.out
        String output = ConsoleInterceptor.copyOut(() -> {
            if (nodes.size() == 1)
                Thread.sleep(20000);    // delay after nodes round
            Thread.sleep(15000);
        });
        System.out.println(output);
        ...
        // revocation of following contract
        Contract revokeContract = ContractsService.createRevocation(simpleContractRevision, key2);
        ...
        output = ConsoleInterceptor.copyOut(() -> {
            registerAndCheckApproved(revokeContract);
            Thread.sleep(5000);
        });
        System.out.println(output);

        // find call follower callback
        assertTrue(output.indexOf("Follower callback received. Revoking ID: " + simpleContractRevision.getId().toString()) >= 0);
        assertTrue(output.indexOf("Follower callback processed. Revoking ID: " + simpleContractRevision.getId().toString()) >= 0);
        ...
        if(envs.size() > 0) {
            for(Long envId : envs) {
                ...
                System.out.println("expected callbacks spent: " + callbackRate * 3);
                System.out.println("found callbacks spent: " + fs.getCallbacksSpent());
                assertEquals(callbackRate * 3, fs.getCallbacksSpent(), 0.001);
            }
        } else {
            fail("FollowerSubscription was not found");
        }
    ...
    @Test
    public void registerFollowerContractInNewItem() throws Exception {
        ...
        // follower contract
        FollowerContract followerContract = ContractsService.createFollowerContract(followerIssuerPrivateKeys, followerIssuerPublicKeys, nodeInfoProvider);
        followerContract.putTrackingOrigin(simpleContract.getOrigin(), "http://localhost:7781/follow.callback", callbackKey.getPublicKey());

        baseContract.addNewItems(followerContract);
        ...
        Set<PrivateKey> stepaPrivateKeys = new HashSet<>();
        stepaPrivateKeys.add(new PrivateKey(Do.read(ROOT_PATH + "keys/stepan_mamontov.private.unikey")));
        Parcel payingParcel = ContractsService.createPayingParcel(baseContract.getTransactionPack(), paymentContract, 1, 200, stepaPrivateKeys, false);
        for (Contract c: baseContract.getNew())
        ...
        assertEquals(followerContract.getCallbackKeys().get("http://localhost:7781/follow.callback"), callbackKey.getPublicKey());
        assertEquals(followerContract.getTrackingOrigins().get(simpleContract.getOrigin()), "http://localhost:7781/follow.callback");
        assertTrue(followerContract.isOriginTracking(simpleContract.getOrigin()));
        ...
        baseContract2.addNewItems(refilledFollowerContract);
        ...
        payingParcel = ContractsService.createPayingParcel(baseContract2.getTransactionPack(), paymentContract, 1, 300, stepaPrivateKeys, false);
        for (Contract c: baseContract2.getNew())
        ...
        ZonedDateTime timeReg2 = ZonedDateTime.ofInstant(Instant.ofEpochSecond(ZonedDateTime.now().toEpochSecond()), ZoneId.systemDefault());
        synchronized (uContractLock) {
            uContract = refilledFollowerContract.getNew().get(0);
        }
        // wait parcel
        node.waitParcel(payingParcel.getId(), 8000);
        // check payment and payload contracts
        assertEquals(ItemState.REVOKED, node.waitItem(payingParcel.getPayment().getContract().getId(), 8000).state);
        assertEquals(ItemState.APPROVED, node.waitItem(payingParcel.getPayload().getContract().getId(), 8000).state);
        assertEquals(ItemState.APPROVED, node.waitItem(refilledFollowerContract.getId(), 8000).state);
        assertEquals(ItemState.APPROVED, node.waitItem(refilledFollowerContract.getNew().get(0).getId(), 8000).state);
        ...
        // init follower callback
        FollowerCallback callback = new FollowerCallback(callbackKey, 7781, "/follow.callback");

        callback.setNetworkNodeKeys(nodes.stream().map(n -> n.getNodeKey()).collect(Collectors.toSet()));

        Contract simpleContractRevision = checkFollowerContractRevision(simpleContract, key2, key, 1, calculateExpires,
                simpleContract.getOrigin(), null, refilledFollowerContract.getTrackingOrigins().size(), true, false, true);
        simpleContractRevision = checkFollowerContractRevision(simpleContractRevision, key, key2, 2, calculateExpires,
                simpleContract.getOrigin(), null, refilledFollowerContract.getTrackingOrigins().size(), false, true, true);

        // check following is new item
        simpleContractRevision = simpleContractRevision.createRevision(key2);
        simpleContractRevision.setOwnerKeys(key);
        simpleContractRevision.seal();
        simpleContractRevision.check();
        simpleContractRevision.traceErrors();
        ...
        Contract baseContract3 = new Contract(key);
        baseContract3.addNewItems(simpleContractRevision);
        baseContract3.seal();
        baseContract3.check();
        baseContract3.traceErrors();
        assertTrue(baseContract3.isOk());

        // intercept System.out
        String output = ConsoleInterceptor.copyOut(() -> {
            registerAndCheckApproved(baseContract3);
            Thread.sleep(5000);
        });
        System.out.println(output);
    ...
    @Test
    public void registerFollowerContractInNewItemBadCreate() throws Exception {
        ...
        // provoke error FAILED_CHECK, "Payment for follower contract is below minimum level of " + nodeConfig.getMinSlotPayment() + "U");
        Parcel payingParcel = ContractsService.createPayingParcel(baseContract.getTransactionPack(), paymentContract, 1, nodeInfoProvider.getMinPayment(followerContract.getExtendedType()) - 1, stepaPrivateKeys, false);
        for (Contract c : baseContract.getNew())
            if (!c.equals(followerContract)) {
                baseContract.getNewItems().remove(c);
                followerContract.addNewItems(c);
    ...
    @Test
    public void registerFollowerContractInNewItemBadUpdate() throws Exception {
        ...
        // provoke error FAILED_CHECK, "Tracking origin is missed"
        refilledFollowerContract.removeTrackingOrigin(simpleContract.getOrigin());
    ...
    private List<Contract> checkFollowerContractRevisionsParallel(Contract contractRevision, Contract contractRevision2, PrivateKey key, PrivateKey key2,
                                                   int callbackNum, ZonedDateTime baseCalculateExpires, ZonedDateTime baseCalculateExpires2, HashId originCallback,
                                                   HashId originCallback2, int origins, boolean waitCallback, boolean checkSubscription) throws Exception {
        // create revision of follow contract
        Contract simpleContractRevision = contractRevision.createRevision(key);
        simpleContractRevision.setOwnerKeys(key2);
        simpleContractRevision.seal();
        simpleContractRevision.check();
        simpleContractRevision.traceErrors();
        assertTrue(simpleContractRevision.isOk());

        // create revision of follow contract 2
        Contract simpleContractRevision2 = contractRevision2.createRevision(key2);
        simpleContractRevision2.setOwnerKeys(key);
        ...
        assertTrue((output.indexOf("Follower callback received. Contract: " + simpleContractRevision.getId().toString()) >= 0) == waitCallback);
        assertTrue((output.indexOf("Follower callback processed. Contract: " + simpleContractRevision.getId().toString()) >= 0) == waitCallback);
        assertTrue((output.indexOf("Follower callback received. Contract: " + simpleContractRevision2.getId().toString()) >= 0) == waitCallback);
        assertTrue((output.indexOf("Follower callback processed. Contract: " + simpleContractRevision2.getId().toString()) >= 0) == waitCallback);
        assertTrue((output.indexOf("Follower callback received. Revoking ID: " + contractRevision.getId().toString()) >= 0) == waitCallback);
        assertTrue((output.indexOf("Follower callback processed. Revoking ID: " + contractRevision.getId().toString()) >= 0) == waitCallback);
        assertTrue((output.indexOf("Follower callback received. Revoking ID: " + contractRevision2.getId().toString()) >= 0) == waitCallback);
        assertTrue((output.indexOf("Follower callback processed. Revoking ID: " + contractRevision2.getId().toString()) >= 0) == waitCallback);
        ...
        long seconds = (long) (days * 24 * 3600);
        ZonedDateTime calculateExpires = baseCalculateExpires.minusSeconds(seconds * callbackNum);
        ZonedDateTime calculateExpires2 = baseCalculateExpires2.minusSeconds(seconds * callbackNum);
        ...
        envs = node.getLedger().getSubscriptionEnviromentIds(originCallback2);
        if(envs.size() > 0) {
            for(Long envId : envs) {
                NImmutableEnvironment environment = node.getLedger().getEnvironment(envId);
                for (ContractSubscription foundCss : environment.subscriptions()) {
                    System.out.println("expected: " + calculateExpires2);
                    System.out.println("found: " + foundCss.expiresAt());
                    assertAlmostSame(calculateExpires2, foundCss.expiresAt(), 5);
                }

                FollowerService fs = environment.getFollowerService();
                System.out.println("expected expiresAt: " + calculateExpires2);
                System.out.println("found expiresAt: " + fs.expiresAt());
                assertAlmostSame(calculateExpires2, fs.expiresAt(), 5);

                System.out.println("expected mutedAt: " + calculateExpires2.minusSeconds(seconds));
                System.out.println("found mutedAt: " + fs.mutedAt());
                assertAlmostSame(calculateExpires2.minusSeconds(seconds), fs.mutedAt(), 5);
        ...
        for (Node networkNode: nodes) {
            ...
            envs = networkNode.getLedger().getSubscriptionEnviromentIds(originCallback2);
            if(envs.size() > 0) {
                for(Long envId : envs) {
                    NImmutableEnvironment environment = networkNode.getLedger().getEnvironment(envId);
                    for (ContractSubscription foundCss : environment.subscriptions()) {
    ...
    @Test
    public void checkFollowerCallbacksParallel() throws Exception {
        ...
        FollowerContract followerContract = ContractsService.createFollowerContract(followerIssuerPrivateKeys,
                followerIssuerPublicKeys, nodeInfoProvider);
        followerContract.putTrackingOrigin(simpleContract.getOrigin(), "http://localhost:7782/follow.callback",
                callbackKey.getPublicKey());
        ...
        assertEquals(followerContract.getCallbackKeys().get("http://localhost:7782/follow.callback"),callbackKey.getPublicKey());
        assertEquals(followerContract.getTrackingOrigins().get(simpleContract.getOrigin()), "http://localhost:7782/follow.callback");
        assertTrue(followerContract.isOriginTracking(simpleContract.getOrigin()));
        assertTrue(followerContract.isCallbackURLUsed("http://localhost:7782/follow.callback"));
        ...
        Contract simpleContract2 = new Contract(key2);
        simpleContract2.addPermission(changeOwnerPerm);
        simpleContract2.getDefinition().getData().set(FOLLOWER_ROLES_FIELD_NAME, followerRoles);
        simpleContract2.seal();
        simpleContract2.check();
        simpleContract2.traceErrors();
        assertTrue(simpleContract2.isOk());
        ...
        FollowerContract followerContract2 = ContractsService.createFollowerContract(followerIssuerPrivateKeys,
                followerIssuerPublicKeys, nodeInfoProvider);
        followerContract2.putTrackingOrigin(simpleContract2.getOrigin(), "http://localhost:7782/follow.callback",
                callbackKey.getPublicKey());

        followerContract2.seal();
        followerContract2.check();
        followerContract2.traceErrors();
        assertTrue(followerContract2.isOk());

        paymentContract = getApprovedUContract();

        payingParcel = ContractsService.createPayingParcel(followerContract2.getTransactionPack(), paymentContract,
                1, 200, stepaPrivateKeys, false);
        ...
        assertEquals(followerContract2.getCallbackKeys().get("http://localhost:7782/follow.callback"), callbackKey.getPublicKey());
        assertEquals(followerContract2.getTrackingOrigins().get(simpleContract2.getOrigin()), "http://localhost:7782/follow.callback");
        assertTrue(followerContract2.isOriginTracking(simpleContract2.getOrigin()));
        assertTrue(followerContract2.isCallbackURLUsed("http://localhost:7782/follow.callback"));
        ...
        itemResult = node.waitItem(followerContract2.getId(), 8000);
        assertEquals("ok", itemResult.extraDataBinder.getBinder("onCreatedResult").getString("status", null));
        ...
        // check if we store environment
        assertNotNull(node.getLedger().getEnvironment(followerContract2.getId()));
        ...
        for (Node networkNode: nodes) {
            ...
            // check if we store environment
            assertNotNull(networkNode.getLedger().getEnvironment(followerContract2.getId()));
        }

        // init follower callback
        FollowerCallback callback = new FollowerCallback(callbackKey, 7782, "/follow.callback");

        callback.setNetworkNodeKeys(nodes.stream().map(n -> n.getNodeKey()).collect(Collectors.toSet()));

        // register 3 revisions with callbacks
        List<Contract> revisions = checkFollowerContractRevisionsParallel(simpleContract, simpleContract2, key, key2, 2, calculateExpires, calculateExpires2,
                simpleContract.getOrigin(), simpleContract2.getOrigin(), 1, true, true);
        revisions = checkFollowerContractRevisionsParallel(revisions.get(0), revisions.get(1), key2, key, 4, calculateExpires, calculateExpires2,
                simpleContract.getOrigin(), simpleContract2.getOrigin(), 1, true, true);
        checkFollowerContractRevisionsParallel(revisions.get(0), revisions.get(1), key, key2, 6, calculateExpires, calculateExpires2,
                simpleContract.getOrigin(), simpleContract2.getOrigin(), 1, true, true);
