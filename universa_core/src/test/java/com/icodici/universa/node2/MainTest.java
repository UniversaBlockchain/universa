...
//                    System.out.println(n.traceItemProcessors());
    ...
    public void perfomanceTest() throws Exception {
        final int MODE_TESTER = 1;
        final int MODE_NETWORK = 2;
        final int MODE_BOTH = 3;
        int mode = MODE_BOTH;
        TestSpace testSpace = null;
        if((mode & MODE_NETWORK) > 0) {
            testSpace = prepareTestSpace();
            testSpace.nodes.forEach(n->n.config.setIsFreeRegistrationsAllowedFromYaml(true));
        }
        ...
        boolean useClientFromTestSpace = false;

        if((mode & MODE_TESTER) > 0) {

            //contracts in pack
            final int PACK_SIZE = 1000;
            //clients per node
            final int CLIENT_PER_NODE = 1;
            ...
            final int CONTRACTS_PER_CLIENT = 10;
            //packs per node
            final int PACKES_COUNT = CLIENT_PER_NODE*NODES_COUNT*CONTRACTS_PER_CLIENT;

            Client[][] clients = new Client[CLIENT_PER_NODE][NODES_COUNT];

            if(CLIENT_PER_NODE == 1 && mode == MODE_BOTH && useClientFromTestSpace) {
                for (int i = 0; i < NODES_COUNT; i++) {
                    clients[0][i] = testSpace.clients.get(i);
                    clients[0][i].ping();
                }
            } else {
                for (int i = 0; i < NODES_COUNT; i++) {
                    clients[0][i] = client.getClient(i);
                    clients[0][i].ping();
                }
                for (int k = 1; k < CLIENT_PER_NODE; k++) {
                    client = new Client(nodeUrl, TestKeys.privateKey(k), null);
                    for (int i = 0; i < NODES_COUNT; i++) {
                        clients[k][i] = client.getClient(i);
                        clients[k][i].ping();
                    }
                }
            }
            ...
            for(int i = 0; i < PACKES_COUNT;i++) {
                contracts.add(createComplexConctract(TestKeys.privateKey(0),PACK_SIZE));
            }
            ...
            for(int k = 0; k < CLIENT_PER_NODE; k++) {
                for(int i =0; i < NODES_COUNT;i++) {
                    int fI = i;
                    int fK = k;
                    ...
                    Thread t = new Thread(() -> {
                        ...
                        for (int j = 0; j < CONTRACTS_PER_CLIENT; j++) {
                            try {
                                clients[fK][fI].register(contracts.get((fK*NODES_COUNT+fI)*CONTRACTS_PER_CLIENT+j).getPackedTransaction());
                            } catch (ClientError clientError) {
                                clientError.printStackTrace();
                            }
                        }
                        System.out.println("Registered " + fK + " " + fI);
                        for (int j = 0; j < CONTRACTS_PER_CLIENT; j++) {
                            try {
                                ItemResult rr;
                                Contract c = contracts.get((fK*NODES_COUNT+fI)*CONTRACTS_PER_CLIENT+j);
                                do {
                                    try {
                                        Thread.sleep(300);
                                    } catch (InterruptedException e) {
                                    ...
                                    rr = clients[fK][fI].getState(c.getId());

                                } while (!rr.state.isApproved());
                            } catch (ClientError clientError) {
                                clientError.printStackTrace();
                            }
                        }
                        System.out.println("Checked " + fK + " " + fI);
                    });
                    threads.add(t);
                    t.start();
                }
            ...
            float tps = PACK_SIZE*PACKES_COUNT / ( (end-start)/1000.0f);
            System.out.println(tps);

        } else {
            //sleep for an hour to let separate tester do the job
